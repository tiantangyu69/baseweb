 provinceArray = new Array("-------请选择-------","北京市","上海市","天津市","重庆市","河北省","山西省","内蒙古自治区","辽宁省","吉林省","黑龙江省","江苏省","浙江省","安徽省","福建省","江西省","山东省","河南省","湖北省","湖南省","广东省","广西壮族自治区","海南省","四川省","贵州省","云南省","西藏自治区","陕西省","甘肃省","宁夏回族自治区","青海省","新疆维吾尔族自治区","香港特别行政区","澳门特别行政区","台湾省","其它");
...
var city = "";
 ...
 cityArray[6] = new Array("内蒙古自治区","呼和浩特|包头|乌海|赤峰|呼伦贝尔盟|阿拉善盟|哲里木盟|兴安盟|乌兰察布盟|锡林郭勒盟|巴彦淖尔盟|伊克昭盟|额尔多斯");
...
function initProvince(p,c){
 document.getElementById("locationP").length = 0 ;
 for(i=0; i<provinceArray.length; i++){
   document.getElementById("locationP").options[i] = new Option(provinceArray[i],provinceArray[i]);
   if(null != p && "" != p) {
	   if(provinceArray[i] == p) {
		   document.getElementById("locationP").options[i].selected = "selected";
	   }
   }
 }
 if(c == null || "" == c || c == "-------请选择-------")
  	 document.getElementById("locationC").options[0] = new Option("-------请选择-------");
 else 
	 //document.getElementById("locationC").options[0] = new Option(c);
 	 city = c;
     getCity(p);
 }
...
function getCity(currProvince)
 {
     ...
     var i,j,k;
     //清空 城市下拉选单
     document.getElementById("locationC").length = 0 ;
           ...
           if(cityArray[i][0]==currProvince)
             {  
                 ...
                 tmpcityArray = cityArray[i][1].split("|")
                   ...
                   {
                     //填充 城市 下拉选单
                     document.getElementById("locationC").options[j] = new Option(tmpcityArray[j],tmpcityArray[j]);
                     if(city == tmpcityArray[j]) {
                    	 document.getElementById("locationC").options[j].selected = "selected";
                     }
                   }
             }
       }
