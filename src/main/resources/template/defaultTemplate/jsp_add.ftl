...
<form action="${'$'}{basePath}/manage/${entityName ? uncap_first}/add.action" method="post" id="add_${entityName ? uncap_first}_form">
    <table class="common_table">
        ...
        <tr>
            ...
            <td width="70%">
                <#if f.type == 'Integer'><ui:numberbox cssClass="input_240" name="${f.name}"/></#if>
                <#if f.type == 'String'><ui:input cssClass="input_240" <#if f.required == true> required="true" validType="length[1,${f.length}]"</#if> <#if f.required == false>validType="length[0,${f.length}]"</#if> name="${f.name}"/></#if>
            </td>
        </tr>
        </#if>
        </#list>
